...
/** Conservative normalization for OCR-derived three-line TD1 candidate text. */
public class Td1MrzNormalizer {
    /** Normalizes [input] without deleting unsupported characters or incomplete content. */
    public fun normalize(input: CharSequence): MrzNormalizationResult {
        ...
        val candidateLines =
            if (preparedLines.size == 1 && preparedLines.single().length == TD1_TOTAL_LENGTH) {
                changes += MrzNormalizationChange(MrzNormalizationChangeType.SINGLE_SEQUENCE_SPLIT)
                preparedLines.single().chunked(TD1_LINE_LENGTH)
            } else {
                preparedLines
            }

        val issues = mutableListOf<MrzValidationIssue>()
        if (candidateLines.size != TD1_LINE_COUNT) {
            issues +=
                MrzValidationIssue(
    ...
    private fun prepareLine(
        ...
        if (withoutSpaces.length != line.length) {
            changes += MrzNormalizationChange(MrzNormalizationChangeType.OCR_SPACES_REMOVED, lineIndex)
        }
        ...
        if (caseFolded) {
            changes += MrzNormalizationChange(MrzNormalizationChangeType.ASCII_CASE_FOLDED, lineIndex)
        }
        return upper
    }
    ...
    private companion object {
        private const val TD1_LINE_COUNT: Int = 3
        private const val TD1_LINE_LENGTH: Int = 30
        private const val TD1_TOTAL_LENGTH: Int = TD1_LINE_COUNT * TD1_LINE_LENGTH
