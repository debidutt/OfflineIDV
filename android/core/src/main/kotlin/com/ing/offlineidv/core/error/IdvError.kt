...
public enum class NfcFailure(
    ...
    UNSUPPORTED_TAG("nfc.unsupported_tag", "This NFC tag is not a supported document chip.", IdvRecovery.RETRY),
    ISO_DEP_UNAVAILABLE("nfc.iso_dep_unavailable", "The document tag does not support IsoDep.", IdvRecovery.RETRY),
    PROTOCOL_UNSUPPORTED(
        "nfc.protocol_unsupported",
        "The ePassport protocol is not available in this build.",
        IdvRecovery.NONE,
    ),
    ACCESS_CONTROL_UNSUPPORTED(
        "nfc.access_control_unsupported",
        "The document chip access method is not supported.",
        IdvRecovery.NONE,
    ),
    CARD_ACCESS_MALFORMED(
        "nfc.card_access_malformed",
        "The document chip access information is malformed.",
        IdvRecovery.RETRY,
    ),
    READ_LIMIT_EXCEEDED(
        "nfc.read_limit_exceeded",
        "The document chip returned more data than allowed.",
        IdvRecovery.NONE,
    ),
    CONNECTION_TIMEOUT("nfc.connection_timeout", "The document-chip connection timed out.", IdvRecovery.RETRY),
    COMMUNICATION_TIMEOUT("nfc.communication_timeout", "Document-chip communication timed out.", IdvRecovery.RETRY),
    TAG_LOST("nfc.tag_lost", "The document-chip connection was lost.", IdvRecovery.RETRY),
    TIMEOUT("nfc.timeout", "The document-chip read timed out.", IdvRecovery.RETRY),
    ACCESS_DENIED("nfc.access_denied", "Document-chip access was denied.", IdvRecovery.RETRY),
    CHIP_DATA_MALFORMED(
        "nfc.chip_data_malformed",
        "The document chip identity data is malformed.",
        IdvRecovery.RETRY,
    ),
    READ_FAILED("nfc.read_failed", "The document chip could not be read.", IdvRecovery.RETRY),
