...
### Milestone 7 — Android NFC integration (historical transport-boundary baseline)
...
At the original M7 gate, the mandatory ePassport-library checkpoint did not approve a packaged protocol implementation. Atlas therefore opened/closed `IsoDep`, transmitted no APDU, never opened the MRZ-derived key, and reported `PROTOCOL_UNSUPPORTED`. The later M7.3 section records the separately approved limited protocol delta; this paragraph remains the historical fallback baseline.
...
- The physical-scan follow-up adds debuggable-build-only, payload-free `MRZ_DIAG` observations at the OCR-to-MRZ boundary. They report counts, candidate lengths, closed stage statuses, and predefined failure reasons only; no MRZ algorithm, threshold, reducer, policy, or release logging behavior changes.
...
Physical-device validation follow-up: Real Android Mode first added a separately composed Netherlands residence-permit profile for standard three-line TD1 MRZ document checks, including a pure TD1 normalizer/parser/validator, policy-free OCR candidate extraction, privacy-safe `TD1` diagnostics, and a host-level document choice. That initial profile was document-only; the later, explicitly approved M7.3 extension below now requires NFC read and printed/DG1 consistency for this profile while keeping face comparison optional. The existing reducer and policy evaluator still select progression and outcome; the shared state-to-UI mapper remains format- and runtime-agnostic. Demo behavior remains unchanged.

This follow-up is not Milestone 8: it adds no selfie capture, face engine, liveness behavior, biometric claim, ePassport protocol, APDU, cryptography, dependency, network access, or persistence. Representative physical validation remains required for the actual Netherlands card layout, OCR crop/framing, and device-specific recognition quality.
...
M7.2 was approval preparation only. At its completion, no approval box was marked, the production runtime still stopped at `PROTOCOL_UNSUPPORTED`, and M7.3 had not started. The later requester authorization and limited implementation are recorded below; formal review boxes and M6/M7 physical-validation debt remain open. Milestone 8 has not started.

### Milestone 7.3 — Limited protected access and DG1 integration (source implementation complete; verification/release gates open)

User authorization on 2026-09-21 accepted the narrowly bounded engineering scope and accepted legal/open-source review as a release blocker. The implementation pins the M7.2 JMRTD 0.8.8 graph; restricts its groups to Maven Central; fails on version conflict; enables dependency locking; contains Scuba APDUs in one `IsoDep` bridge; suppresses JMRTD/Scuba/Bouncy Castle JUL output; selects a closed PACE ECDH-GM AES allowlist before an explicit BAC exception; prevents BAC after a PACE attempt; bounds CardAccess to 64 KiB, DG1 to 4 KiB, and each command/response frame to the smaller of the device maximum and 64 KiB; extracts only document number, nationality, birth date, and expiry date; and reports only existing safe NFC observations.

Both TD3 and TD1 OCR pipelines now retain the same minimal access/comparison fields rather than raw MRZ lines. The Netherlands residence-permit profile requires NFC read plus printed/DG1 consistency and does not require face comparison. Shared verification mapping uses document-neutral chip copy. The reducer, `VerificationPolicyEvaluator`, verification evidence semantics, retry behavior, operation tokens, orchestration, Demo engines, camera/OCR behavior, and navigation architecture are unchanged.

The scope explicitly excludes DG2, portrait extraction, Passive Authentication/SOD/CSCA, Chip/Active/Terminal Authentication, face matching, liveness, custom cryptography, networking, persistence, and any authenticity or holder-identity claim. Milestone 8 has not started.

Implementation tests add fail-closed PACE/BAC selection, LDS byte limits, minimal TD1 access artifacts, TD1/TD3-neutral comparison, exact dependency configuration, protocol/APDU containment, runtime policy isolation, residence-profile requirements, and document-neutral NFC UI copy. The repository source now defines 395 JVM and 34 instrumentation test methods. The Codex execution sandbox cannot start Gradle 9.6.1 because `FileLockContentionHandler` is denied its local UDP socket before project configuration. Therefore compilation, resolved dependency locks/verification metadata, JVM tests, lint, debug/release D8/R8, APK assembly, SBOM, packaged manifest/binary inspection, and physical PACE/BAC/DG1 execution remain unverified here and are blocking—not waived or reported as passes.

M7.3 release exit criteria are exact: run `spotlessCheck`, `test`, `lint`, `assembleDebug`, and `:android:app-demo:assembleDebug` successfully; generate/review the complete lockfile and strict verification metadata against the approved checksums; prove the resolved graph contains only the five reviewed artifacts and excludes legacy bcprov; complete debug/release D8/R8, SBOM, notice/license, permission, offline/network, sensitive-log, exception, and APDU-listener audits; obtain legal/open-source approval for every distribution form; and execute the blocking device matrix with an authorized runtime-only Netherlands permit plus representative passport coverage, including PACE, permitted BAC, mismatch, malformed/oversized data, tag loss, timeout, cancellation, Activity recreation, cleanup, and airplane mode. Until all pass, M7.3 is not release-ready and Milestone 8 is not ready to start.

### Milestone 7.4 — Bounded Netherlands residence-permit chip authenticity (source implementation complete; verification/release gates open)

Requester confirmation on 2026-09-22 authorized the recommended bounded scope: offline and fail-closed Netherlands residence-permit Passive Authentication plus PA-bound Chip Authentication, with signed-data authenticity and live-chip proof kept as separate evidence. The implementation reads SOD under 1 MiB and DG14 under 64 KiB; verifies DG1/DG14 hashes, signed attributes, SOD signature, signer identity, reviewed digest/signature/key policy, DSC validity/purpose, and a direct DSC signature under a bundled fingerprint-pinned Netherlands residence-permit CSCA; then selects a single reviewed ECDH/AES Chip Authentication version 1 suite/key and delegates fresh key agreement and secure-messaging transition to JMRTD.

The trust snapshot contains only Netherlands residence-permit CSCA serial 4 from the official Netherlands PKD certificate export, pinned to SHA-256 `0F:D7:E3:BE:92:3B:DB:E8:3B:4E:4B:08:E2:59:74:65:5B:5E:55:F6:61:44:8B:9A:FA:A4:A7:8F:76:0E:D2:BC`. It is valid for Atlas use from 2026-09-22 inclusive until 2027-01-22 exclusive, then fails closed. No runtime trust download occurs. An official CRL could not be acquired in the implementation environment, so revocation is not evaluated; Atlas's bounded `VALID` observation does not assert that the DSC is unrevoked. Production trust/revocation governance and broader issuer/rollover coverage remain blockers.

The Netherlands residence-permit Real Android profile requires NFC, printed/DG1 consistency, PA, and CA and still omits face matching. The policy evaluator adds only the explicit interpretation requested by that profile: an explicit required PA/CA failure rejects, while missing/unavailable/unsupported/stale proof is inconclusive. The reducer adds separate PA/CA evidence; it remains unaware of JMRTD, certificates, crypto providers, Android NFC, and document format. The shared mapper remains runtime-mode agnostic and renders `Signed chip data` separately from `Live chip proof`.

M7.4 explicitly excludes DG2, portrait extraction, Active Authentication, Terminal Authentication, face matching, liveness, custom cryptographic primitives, runtime networking, persistence, and Milestone 8. Source tests cover trust fingerprint/freshness, closed-suite/key/signature-parameter selection, owned-buffer erasure, distinct PA/CA observations/evidence/policy/UI, residence-profile requirements, and protocol/policy containment. The repository now defines 407 JVM and 34 instrumentation test methods (441 total). Gradle tasks remain blocked in the Codex sandbox before project configuration because Gradle 9.6.1 cannot bind its local file-lock UDP socket; their required external execution is not waived.

M7.4 release exit criteria are exact: all M7.3 gates remain satisfied; `spotlessCheck`, `test`, `lint`, `assembleDebug`, and `:android:app-demo:assembleDebug` pass on the final source; strict dependency locks and verification metadata match the reviewed graph; an approved, current, rollback-resistant Netherlands trust snapshot and revocation policy/material replace or explicitly govern the prototype limitation; independent security/crypto, PKI, privacy, legal/open-source, architecture, and Android/release reviews approve the final artifact; malformed SOD/DG14/certificate and algorithm-confusion fuzz suites pass; debug and minified release builds prove no sensitive logging/network/APDU transcript; and the blocking matrix passes on the requester's authorized residence permit and representative devices, including actual DSC/CSCA coverage, PA, CA suite/version, tag loss, timeout, cancellation, Activity recreation, cleanup, repeat scans, and airplane mode. Until then, M7.4 is source-implemented but not release-ready, and Milestone 8 must not start.
...
Explicitly deferred after Milestone 6: Android NFC/`IsoDep`, BAC/PACE and chip reads, real selfie capture, face matching or liveness, secure persistence, Hilt, network services, production analytics, screenshot/root/debug hardening, and conference timing/script polish. Android NFC/`IsoDep` transport was subsequently delivered in Milestone 7, the separately authorized M7.3 follow-up added the limited PACE/BAC plus bounded-DG1 path, and M7.4 added the bounded Netherlands PA/CA path described above. DG2, broad governed trust/revocation coverage, Active/Terminal Authentication, face work, and the remaining hardening items stay deferred. Milestone 8 has not started.
...
The separately authorized M7.3 follow-up added the pinned protocol graph, PACE-first/BAC protected access, bounded DG1 reads, TD1 support, and printed/DG1 consistency. M7.4 then added bounded SOD/DG14 reads, the single-anchor Netherlands residence-permit trust snapshot, Passive Authentication, separate PA-bound Chip Authentication evidence, and corresponding real-profile policy. Still deferred after Milestone 7.4: DG2, governed broad CSCA/link/rollover/revocation material, Active/Terminal Authentication, real selfie capture, face matching or liveness, secure persistence, Hilt, network services, production analytics, screenshot/root/debug hardening, and conference timing/script polish. Milestone 8 has not started.

## Exact Milestone 7.4 file groups

- NFC source/resources: finite Chip Authentication observation, PA/CA-preserving chip validation, bounded owned-byte handling, SOD/DG14 authenticity adapter, strict PSS and CA suite/key policy, fingerprint-pinned Netherlands residence-permit CSCA, and PA/CA orchestration inside `android/nfc`.
- Verification/app/UI source: separate CA status/evidence/capability, policy prerequisite and interpretation, reducer evidence mapping, Real Android residence-profile requirement, safe effect translation, and distinct implementation-neutral `Signed chip data`/`Live chip proof` rows.
- Tests: snapshot fingerprint/freshness, strict CA selection, PSS parameter policy, owned-buffer erasure, PA/CA artifact/evidence/policy/UI separation, profile requirements, and platform/policy/fake isolation.
- Architecture/security: ADR 0017, Android NFC/overview/UI/camera boundaries, prototype trust addendum, threat model, approval history, device matrix, third-party/trust-material notice, README, and this plan.

Milestone 7.4 does not include Milestone 8 work.
