...
VerificationStateMachine
  ...
  -> AndroidNfcTagDiscovery keeps reader mode plus foreground dispatch enabled while the host is resumed and the read is active
  -> ReaderCallback or the foreground-dispatch fallback keeps Tag inside the Android adapter
...
The Activity attaches to the discovery adapter while resumed and detaches while paused. The adapter enables Android reader mode and an ISO-DEP-filtered foreground-dispatch fallback only when all three conditions hold: an Activity is attached, NFC is available, and one read is active. Both remain enabled after a tag is claimed so Android keeps a discovery route and RF field available through `IsoDep.connect()` and the complete protected chip read. They are disabled only when that read is cancelled, completes, is replaced, the host detaches, or the adapter closes. Detach also closes a claimed physical-tag lease, including the detach/discovery race; an active read that has not discovered a tag can resume discovery when the host reattaches. Activity recreation can reattach the same ViewModel-owned runtime without placing an Activity, `Intent`, `Tag`, or `IsoDep` in reducer or Compose state.

The fallback addresses a physical-device failure observed on a Samsung SM-F936B running Android 16. The NFC event log records Atlas registering flags `131` (NFC-A, NFC-B, skip-NDEF) and the privileged Android process registering flags `4096` 23 ms later. Android defines `4096` as forced polling disable. The same dump reported `mOverrideIntent=null`, confirming that the previous implementation had no foreground-dispatch route. The system registration replaces or suppresses the app reader callback even though `enableReaderMode` returned normally, which explains why only reader-mode STARTED/SUCCEEDED diagnostics appeared and no tag callback followed. Foreground dispatch remains registered independently, so when system polling resumes the physical ISO-DEP tag enters the same existing one-shot discovery path. It does not bypass system-wide polling suppression, fabricate a tag, or alter BAC/PACE, APDU, evidence, reducer, retry, or policy behavior.
